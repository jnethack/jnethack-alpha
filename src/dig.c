...
*/
            You("%sに%sを%s．", surface_type, tname, in_thru);
    } else if (!madeby_obj && canseemon(madeby)) {
#if 0 /*JP:T*/
        pline("%s digs %s %s the %s.", Monnam(madeby), an(tname), in_thru,
              surface_type);
#else
        pline("%sは%sに%sを作った．", Monnam(madeby),
              surface_type, tname);
#endif
...
*/
            pline_The("%sが崩れ落ちて，%sができた．", surface_type, tname);
        else
/*JP
            pline("%s appears in the %s.", An(tname), surface_type);
*/
            pline("%sが%sに現われた．", tname, surface_type);
    }
...
*/
                ...
                You("%sのを%sした．", d_action2[dig_target],
                    svc.context.digging.chew ? "開始" : "再開");
#endif
...
bury_objs(int x, int y)
{
    ...
    if (costly && loss) {
        ...
        You("owe %s %ld %s for burying merchandise.", shkname(shkp), loss,
            currency(loss));
#else
        You("埋まった商品に関して%sに%ld%sの負債を負った．", shkname(shkp), loss,
            currency(loss));
#endif
    }
}
