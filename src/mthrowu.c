...
#else
            if (!(broken = flooreffects(obj, x, y, "落ちる"))) {
#endif
        ...
        if (vis) {
            if (otmp->otyp == EGG) {
                ...
                pline("ビチャッ！%sは%s卵に当たった！", Monnam(mtmp),
                      otmp->known ? s_suffix(mons[otmp->corpsenm].pmnames[NEUTRAL])
                                  : "");
#endif
            } else {
                char how[BUFSZ];
...
*/
                    m_name = strcat(s_suffix(m_name), "体");
/*JP
...
#else
                pline("%sは%sによって目が見えなくなった．", Monnam(mtmp),
                      the((otmp->oclass == VENOM_CLASS) ? "毒液"
                          : (otmp->otyp == CREAM_PIE) ? "パイ"
                            : xname(otmp))); /* catchall; not used */
#endif
            mtmp->mcansee = 0;
...
*/
            You("%sをつかまえた．", gem_xname);
#if 0 /*JP:T*/
            You("are not interested in %s junk.", mon_s_name);
#else
            You("%sガラクタに興味はない．", mon_s_name);
#endif
            makeknown(gem->otyp);
            dropy(gem);
        } else {
            ...
            You("accept %s gift in the spirit in which it was intended.",
                mon_s_name);
#else
            You("これが欲しかったんだと思いながら%s贈り物を受けとった．",
                mon_s_name);
#endif
#if 0 /*JP:T*/
            (void) hold_another_object(gem, "You catch, but drop, %s.",
                                       gem_xname, "You catch:");
#else
            (void) hold_another_object(gem, "あなたは%sを捕ったが，落した．",
                                       gem_xname, "を捕った．");
#endif
        }
        return TRUE;
...
*/
                ...
                You_hear(Hallucination ? "angry snakes!"
                                       : "a hissing noise.");
#else
                You_hear(Hallucination ? "怒ったへびの声を聞いた！"
                                       : "シューッという音を聞いた．");
#endif
            }
            if (!nodissolve)
...
#else /*それぞれ、金属的な響き/鋭く強い打撃/軽くて柔らかい/軽くて高い/重く鈍い*/
                "", "ぐわーん", "バシッ", "バサッ", "チャリン", "ゴツン"
#endif
            };
