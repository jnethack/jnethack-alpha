...
#if 1 /*JP*/
staticfn char *
get_table_str_ic(lua_State *L, const char *name)
{
    char *p;
    char *ret;

    p = get_table_str(L, name);
    ret = dupstr(utf8toic(p));
    free(p);
    return ret;
}
#endif
...
#else
            br_name = get_table_str_ic(L, "name");
#endif
...
#else
    dgn_name = get_table_str_ic(L, "name");
#endif
