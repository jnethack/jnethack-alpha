...
#else
static const char whistle_str[] = "%s音をたてた．",
                  alt_whistle_str[] = "%s振動をおこした．";
#endif
...
*/
        ...
        if (otmp)
            ...
            pline("To attach candles, apply them instead of the %s.",
                  xname(obj));
#else
            pline("ろうそくを取り付けるには，%sではなくろうそくを使ってください．",
                  xname(obj));
#endif
        return;
    }
...
#else
    const char *lamp = (obj->otyp == OIL_LAMP
                        || obj->otyp == MAGIC_LAMP) ? "ランプ"
                       : (obj->otyp == BRASS_LANTERN) ? "ランタン"
                         : NULL;
#endif
...
*/
            pline("%s%sの灯は消えた．", Shk_Your(buf, obj), lamp);
        else
/*JP
...
*/
            pline("%s%sに灯が灯った．", Shk_Your(buf, obj), lamp);
        } else { /* candle(s) */
#if 0 /*JP:T*/
...
#else
            You("小手なしで%sを缶詰にしようとした．", corpse_name);
#endif
            kbuf[0] = '\0';
        } else {
#if 0 /*JP:T*/
...
#else
            Sprintf(kbuf, "小手をつけずに%sを缶詰にしようとして", corpse_name);
#endif
        }
        instapetrify(kbuf);
    }
...
*/
                ...
                You("%sが%sから%sのを見た！%s", monnambuf,
                        carriedby, jpast(locomotion(mtmp->data, "落ちる")),
                        and_vanish);
#endif
            }
            break;
...
#else
        what = stway->isladder ? "はしごの上では" : "階段の上では";
#endif
...
*/
                    You("%sを落とした！", trapname(ttyp, FALSE));
                    dropx(otmp);
                    return;
                }
            }
...
*/
    You("%s%sを仕掛けはじめた．", shk_your(buf, otmp), trapname(ttyp, FALSE));
...
*/
            You("%sを仕掛け終えた．", trapname(ttyp, FALSE));
...
*/
        ...
        const char *wrapped_what = sobj_at(BOULDER, rx, ry) ? "a boulder"
                                   : IS_FURNITURE(levl[rx][ry].typ)
                                     ? something : (char *) 0;
#else
        const char *wrapped_what = sobj_at(BOULDER, rx, ry) ? "岩"
                                   : IS_FURNITURE(levl[rx][ry].typ)
                                     ? something : (char *) 0;
#endif
...
*/
    const char thump[] = "ゴツン！%sは傷つかなかった．";
...
#else
    boolean is_fragile = (objdescr_is(obj, "バルサの杖")
                          || objdescr_is(obj, "ガラスの杖"));
#endif
