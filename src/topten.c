...
*/
    ...
    {
        char *dst = eos(buf);
        const char *src = kname;

        while (*src && (size_t) (dst - buf) < siz - 1) {
            char c = *src++;
            if (c == ',')
                c = ';';
            else if (c == '=')
        ...
        *dst = '\0';
        /* 多バイト文字の途中で切れた場合に末尾を削る */
        truncate_japanese(buf, (int) strlen(buf));
    }
    siz -= strlen(buf);
#endif
...
#else
        Fprintf(rfile, "%cwhile=%s", XLOG_SEP,
                gm.multi_reason ? gm.multi_reason : "無力な間に");
#endif
...
/*JP: 日本語では「○○を手に」を先に追加しないと不自然 */
    jdeath = t1->death;
    if (!STRNCMPEX(jdeath, "魔除けを手に"))
        jdeath += l;
    else if (!STRNCMPEX(jdeath, "天上で恥辱を受け"))
        jdeath += l;
    else if (!STRNCMPEX(jdeath, "偽物の魔除けを掴まされ"))
        jdeath += l;
#endif
#if 0 /*JP*/
    if (!strncmp("escaped", t1->death, 7)) {
#else
    if (!STRNCMP2(jdeath, "脱出した")
...
#else
            Sprintf(eos(linebuf), "%sにて", arg);
#endif
        } else {
/*JP
            Sprintf(eos(linebuf), " in %s", svd.dungeons[t1->deathdnum].dname);
*/
            Sprintf(eos(linebuf), "%s", svd.dungeons[t1->deathdnum].dname);
...
prscore(int argc, char **argv)
{
    ...
    if (argc <= 1) {
        ...
        for (j = 0; j < argc; j++) {
            const char *pp = str2ic(argv[j]);
            players[j] = (char *)alloc(strlen(pp) + 1);
            strcpy((char *)players[j], pp);
        }
        players[j] = NULL;
#endif
    }
