...
#else
            Sprintf(svk.killer.name, "%sの肉を食べ",
                    mons[pm].pmnames[NEUTRAL]);
#endif
            svk.killer.format = KILLED_BY;
/*JP
...
*/
            gm.multi_reason = "金貨の山の真似をしている時に";
            Sprintf(buf,
                    Hallucination
/*JP
...
#if 1 /*JP*/
/* tintxts[].txt が「の」で始まるなら怪物名の後ろに置く */
staticfn boolean
tintxt_is_prefix(int r)
{
    return (boolean) (strncmp(tintxts[r].txt, "の", strlen("の")) != 0);
}
#endif
...
#else
    pline("ゲェ！%s%sだ！",
          is_rottable(obj) ? "腐った" : "恐ろしい", foodword(obj));
#endif
    if (!rn2(4)) {
        if (Hallucination)
/*JP
...
*/
        gm.multi_reason = "腐った食べ物で意識を失っている間に";
/*JP
        gn.nomovemsg = "You are conscious again.";
*/
        gn.nomovemsg = "あなたはまた正気づいた．";
...
*/
        Snprintf(buf, sizeof buf, "%sはあなたになじまないようなにおいがする．", foodsmell);
    }
