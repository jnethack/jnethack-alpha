...
#else
static const char invisexplain[] = "覚えているが見えていない怪物",
...
#if 1 /*JP*//*多バイト文字の途中で切らない*/
    if (space_left < (sizeof sep - 1) + strlen(new_str))
        return 0;
#endif
...
#else
        Sprintf(eos(outbuf), "，%sとつながっている",
                uball ? ansimpleoname(uball) : "無？");
#endif
...
#else
        Strcpy(outbuf, "罠の仕掛けられた箱"); /* might actually be a large box */
#endif
    else if (trapped_door_at(tnum, x, y))
#if 0 /*JP:T*/
        Strcpy(outbuf, "trapped door"); /* not "trap door"... */
#else
        Strcpy(outbuf, "罠の仕掛けられた扉"); /* not "trap door"... */
#endif
    else
...
#else
            Snprintf(eos(outbuf), BUFSZ - buflen, "，%sの雲の中",
                     poison_gas ? "毒ガス" : "蒸気");
#endif
        }
    }
...
#else
        Snprintf(eos(buf), BUFSZ - strlen(buf), "木に%sいる",
                 (otmp && is_treefruit(otmp)) ? "ぶら下がって" : "引っかかって");
#endif
...
*/
        Strcat(buf, "，動けない (麻痺しているか眠っているか作業中)");
...
*/
            Sprintf(eos(buf), "，%sに捕まっている", trapname(tt, FALSE));
            t->tseen = 1;
        }
    }
...
*/
        Snprintf(pooltype, sizeof pooltype, "どろどろの%s", hliquid("溶岩"));
        return pooltype;
    } else if (ltyp == ICE) {
        if (!hallucinate)
/*JP
...
*/
        Snprintf(pooltype, sizeof pooltype, "凍った%s", hliquid("水"));
        return pooltype;
    } else if (ltyp == POOL) {
/*JP
        Snprintf(pooltype, sizeof pooltype, "pool of %s", hliquid("water"));
*/
        Snprintf(pooltype, sizeof pooltype, "%sたまり", hliquid("水"));
        return pooltype;
    } else if (ltyp == MOAT) {
...
*/
            Snprintf(pooltype, sizeof pooltype, "深い%s", hliquid("水"));
            return pooltype;
...
#else
        "固い",           /* 0: not melting */
        "しっかりした",   /* 1: more than 1000 turns left */
        "安定した",       /* 2: 101..1000 turns left */
        "不安定な",       /* 3:  51..100 turns left */
        "薄い",           /* 4:  15..50 turns left */
        "シャリシャリの", /* 5:   1..14 turns left; matches Warning on ice */
#endif
    };
...
*/
        Sprintf(buf, "%sの内部", mon_nam(u.ustuck));
        pm = u.ustuck->data;
                ...
                if (!user_typed_name && !without_asking) {
                    ...
                    copynchars(eos(question), entrytext,
                               (int) (sizeof question - 1
                                      - (strlen(question)
                                        + sizeof "」の詳細を見る？")));
                    truncate_japanese(question, (int) strlen(question));                                        
        ...
        if (sym == (looked ? gs.showsyms[alt_i] : defsyms[alt_i].sym)) {
            ...
            article = strstri(x_str, " of a room") ? 2
                      : !(alt_i == S_stone
                          || strcmp(x_str, "air") == 0
                          || strcmp(x_str, "land") == 0);
#else /*不使用だが未定義警告を避けるために初期化だけしておく*/
            article = 0;
#endif
...
#else
            if (!strcmp(look_buf, "下り階段")
                && on_level(&u.uz, &qstart_level) && !ok_to_quest())
#endif
/*JP
                Strcpy(look_buf, "blocked staircase down");
*/
                Strcpy(look_buf, "通れない下り階段");
...
*/
                Snprintf(temp_buf, sizeof temp_buf, " [視覚: %s]", monbuf);
                (void) strncat(out_str, temp_buf,
                               BUFSZ - strlen(out_str) - 1);
            }
        }
    ...
    boolean force) /* True: '/e' or '/E', False: '//' or ';' */
{
...
#else
    boolean floorengr = !strcmp(buf, " (刻まれた文字"),
            headstone = !strcmp(buf, " (墓");
#endif
    ...
    if (ep->eread)
        Snprintf(temp_buf, sizeof temp_buf, " %s：「%s」",
                 headstone ? "墓石の文字" : "覚えている文字",
                 ep->engr_txt[remembered_text]);
    else
        Snprintf(temp_buf, sizeof temp_buf, "はまだ読んでいない");
#endif
...
*/
const char what_is_a_location[] = "怪物，物，場所";
...
*/
                     clr, "地図上にあるもの", MENU_ITEMFLAGS_NONE);
            any.a_char = 'i';
            add_menu(win, &nul_glyphinfo, &any,
...
*/
                     clr, "あなたが持っているもの", MENU_ITEMFLAGS_NONE);
            any.a_char = '?';
            add_menu(win, &nul_glyphinfo, &any,
                     flags.lootabc ? 0 : any.a_char,
...
*/
                         clr, "近くにいる怪物", MENU_ITEMFLAGS_NONE);
                any.a_char = 'M';
                add_menu(win, &nul_glyphinfo, &any,
                         flags.lootabc ? 0 : any.a_char,
                         flags.lootabc ? any.a_char : 0, ATR_NONE,
...
*/
                         clr, "近くにある物", MENU_ITEMFLAGS_NONE);
                any.a_char = 'O';
                add_menu(win, &nul_glyphinfo, &any,
                         flags.lootabc ? 0 : any.a_char,
                         flags.lootabc ? any.a_char : 0, ATR_NONE,
...
*/
                         clr, "近くにある罠", MENU_ITEMFLAGS_NONE);
                any.a_char = 'T';
                add_menu(win, &nul_glyphinfo, &any,
                         flags.lootabc ? 0 : any.a_char,
                         flags.lootabc ? any.a_char : '\"', ATR_NONE,
...
*/
                         clr, "近くにある文字", MENU_ITEMFLAGS_NONE);
                any.a_char = 'E';
                add_menu(win, &nul_glyphinfo, &any,
                         flags.lootabc ? 0 : any.a_char,
                         flags.lootabc ? any.a_char : '|', ATR_NONE,
...
*/
                    pline("%sを指定してください．", what_is_a_location);
...
#else
                Sprintf(eos(lookbuf), "，%sで隠されている", encglyph(glyph));
#endif
...
#else
                    Sprintf(outbuf, "%sで見た，あるいは覚えている罠：",
                            nearby ? "近く" : "このレベル");
#endif
...
*/
        pline("%s見た，あるいは覚えている罠はない．", nearby ? "近くには" : "");
    destroy_nhwindow(win);
}
...
#else
            Sprintf(lookbuf, "%s", is_headstone ? "墓" : "刻まれた文字");
#endif
            ...
            if (*lookbuf) { /* (redundant) */
                ...
                if (count == 1) {
                    ...
                    Sprintf(outbuf, "%sseen or remembered engravings%s:",
                            ...
                            nearby ? "" : " on this level");
#else
                    Sprintf(outbuf, "%sで見た，あるいは覚えている刻み文字：",
                            nearby ? "近く" : "このレベル");
#endif
                    putstr(win, 0, upstart(outbuf));
                    /* hack alert! Qt watches a text window for any line
...
*/
        pline("%s見た，あるいは覚えている刻み文字はない．", nearby ? "近くには" : "");
    destroy_nhwindow(win);
}
...
*/
    q = yn_function("どういうコマンド？", (char *) 0, '\0', TRUE);
#ifdef ALTMETA
...
*/
    { dispfile_optmenu, "オプションの設定に%sコマンドを使う" },
/*JP
...
*/
    { dispfile_usagehelp, "JNetHackのコマンドラインの説明" },
/*JP
