...
*/
            Strcpy(svk.killer.name, "三途の川の渡し船に乗っている間に抜けた");
        }
    }
...
*/
        Strcat(svk.killer.name, "(魔除けを手にしていた)");
    } else if (how == ESCAPED) {
        if (Is_astralevel(&u.uz)) /* offered Amulet to wrong deity */
/*JP
            Strcat(svk.killer.name, " (in celestial disgrace)");
*/
            Strcat(svk.killer.name, "(天上で恥辱を受けた)");
        else if (carrying(FAKE_AMULET_OF_YENDOR))
/*JP
            Strcat(svk.killer.name, " (with a fake Amulet)");
*/
            Strcat(svk.killer.name, "(偽物の魔除けを掴まされていた)");
...
#else
        Sprintf(eos(pbuf), "%ldポイントをマークし%s．",
                u.urexp,
                (how == ASCENDED) ? "昇天した"
                                  : "迷宮から脱出した");
#endif
