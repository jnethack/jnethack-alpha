...
*/
    gm.multi_reason = "死ぬほど驚いた隙に";
/*JP
    gn.nomovemsg = "You regain your composure.";
*/
    gn.nomovemsg = "あなたは平静を取り戻した．";
}
...
*/
    } else if (objdescr_is(otmp, "煙がでている")
...
*/
        ...
        pline("ワーオ！気分が%sなった！",
              (!otmp->blessed) ? "よく"
              : unfixable_trouble_count(FALSE) ? "だいぶよく"
                : "とてもよく");
#endif
...
static const char *bottlenames[] = { "bottle", "phial", "flagon", "carafe",
                              "flask",  "jar",   "vial" };
#else
static const char *bottlenames[] = { "瓶", "玻璃瓶", "一升瓶", "水差し",
                              "フラスコ",  "壷",   "ガラス瓶" };
#endif
...
*/
            pline("%s%sは濡れた．", buf, xname(saddle));
    } else {
...
*/
                pline("%sは一瞬透明になったようだ．", Monnam(mon));
...
*/
        Snprintf(obuf, sizeof obuf, "あなたの%s", body_part(HAND));
    } else {
#if 0 /*JP:T*/
 ...
 [abdeghjkmnpqstvwyzBCEFHIKLNOQRTUWXZ#-# or ?*] "));
    ...
    Snprintf(qbuf, sizeof qbuf, "dip into");
#endif
...
#else
        hold_potion(obj, "%sを落した！", doname(obj), (const char *) 0);
#endif
        return ECMD_TIME;
    }
...
*/
                Sprintf(buf, "%sの一つ", xname(potion));
            else
                Strcpy(buf, The(xname(potion)));
/*JP
            pline("%s forms a coating on %s.", buf, the(xname(obj)));
*/
            pline("%sが%sに塗られた．", buf, xname(obj));
            obj->opoisoned = TRUE;
            poof(potion);
            return ECMD_TIME;
...
#else
        hold_potion(singlepotion, "お手玉して%sを落としてしまった！",
                    doname(singlepotion), (const char *) 0);
#endif
        return ECMD_TIME;
    }
