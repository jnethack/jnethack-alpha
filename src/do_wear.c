...
#else
staticfn void already_wearing(const char *, struct obj *);
#endif
...
*/
            ...
            You("%s%s are noisy.", riding ? "and " : "sure",
                riding ? x_monnam(u.usteed, ARTICLE_YOUR, (char *) NULL,
                       ...
                       : "");
#else
            pline("あなた%s%sは足音が大きくなった．", riding ? "と" : "",
                riding ? x_monnam(u.usteed, ARTICLE_YOUR, (char *) NULL,
                                  (SUPPRESS_SADDLE | SUPPRESS_HALLUCINATION),
                                  FALSE)
                       : "");
...
#else
        Snprintf(kbuf, sizeof kbuf, "%sを持っているときに%s",
                 killer_xname(obj), hbuf);
#endif
        instapetrify(kbuf);
    ...
    if (obj == uskin
        ...
        if (obj != uskin) {
            ...
            Snprintf(why, sizeof why, " without taking off your %s first",
                     what);
#else
            Snprintf(why, sizeof why, "先に%sをはずさないと", what);
#endif
        } else {
#if 0 /*JP*/
...
#else
already_wearing(const char *cc, struct obj *otmp)
#endif
{
#if 1 /*JP*/
    const char *j;
...
#else
    n = query_objlist("どの装備を解きますか？", &gi.invent,
                      (SIGNAL_NOMENU | USE_INVLET | INVORDER_SORT),
                      &pick_list, PICK_ANY,
                      all_worn_categories ? is_worn : is_worn_by_type);
#endif
...
disintegrate_arm(struct obj *atmp)
{
    ...
    if ((otmp = maybe_destroy_armor(uarmc, atmp, &resistedc)) != 0) {
        ...
        urgent_pline("Your %s %s to dust and %s to the %s!",
                     ...
                     surface(u.ux, u.uy));
#else
        urgent_pline("あなたの%sは塵となり%sに落ちた！",
                     /* suit might be "dragon scales" so vtense() is needed */
...
#else
        urgent_pline("あなたの%sはズタズタに裂け，小さな糸屑となり落ちた！",
                     shirt_simple_name(otmp)); /* always "shirt" */
#endif
...
#else
        urgent_pline("あなたの%sは消えた！", gloves_simple_name(otmp));
#endif
        losing_gloves = TRUE;
