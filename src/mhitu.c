...
#else
        pline_mon(mtmp, "%sはあなた%s%s．", Monst_name,
...
#else
        pline_mon(mtmp, "%s%s%s", Monst_name, verb, punct);
#endif
    }
...
*/
        pline_mon(mtmp, "%sは友好的なふりをしている．",Monnam(mtmp));
    else
#if 0 /*JP:T*/
        pline_mon(mtmp, "%s %smisses!", Monnam(mtmp),
                  (nearmiss && flags.verbose) ? "just " : "");
#else
        pline_mon(mtmp, "%sの攻撃は%sた！", Monnam(mtmp),
...
#else
        pline_mon(mtmp, "%sは%s%s%s．", Monnam(mtmp),
                  xname(otemp), 
                  (otemp->quan > 1L) ? "のひとつ" : "",
                  mswings_verb(otemp, bash));
#endif
...
*/
                pline_mon(mtmp, "あなたが落ちることで%sは引き戻された！", Monnam(mtmp));
                return 0;
            }
...
*/
            pline_mon(mtmp, "%sは締めつけをわずかに緩めた．", Monnam(mtmp));
        } else if (!range2) {
...
*/
                    pline_mon(mtmp, "%sは息を吸いこんだ！", Monnam(mtmp));
                } else {
                    if (youseeit) {
#if 0 /*JP:T*/
    ...
    if (!u.uswallow) { /* swallows you */
        ...
        if (u.usteed) {
            ...
            urgent_pline("%s %s forward and plucks you off %s!",
                         ...
                         buf);
#else /*「突進」でシンプルに処理*/
            urgent_pline("%sは突進しあなたを%sから引き落した！",
            ...
            urgent_pline("%s %s!", Monnam(mtmp),
                         ...
                         : enfolds(mtmp->data) ? "folds itself around you"
                           : "engulfs you");
#else
            urgent_pline("%sはあなた%s！", Monnam(mtmp),
                         digests(mtmp->data) ? "をまるごと巻き込んだ"
                         : enfolds(mtmp->data) ? "の周りを包み込んだ"
                           : "を飲み込んだ");
#endif
        }
        stop_occupation();
    ...
    if (!u.uswallow) {
        ...
        You("get %s!", digests(mtmp->data) ? "regurgitated"
                       : enfolds(mtmp->data) ? "released"
                         : "expelled");
#else
        You("%sされた！", digests(mtmp->data) ? "吐き戻"
                       : enfolds(mtmp->data) ? "解放"
                         : "排出");
#endif
        if (flags.verbose
    ...
    boolean ufound)
{
    ...
    if (!ufound) {
        ...
        pline("%sは何もない%sで爆発した！",
              ...
              is_waterwall(mtmp->mux,mtmp->muy) ? "水中" : "空間");
#endif
    } else {
        hitmsg(mtmp, mattk);
    }
...
*/
                pline_mon(mtmp, "%sは石になった！", Monnam(mtmp));
            gs.stoned = TRUE;
            killed(mtmp);
...
*/
                pline_mon(mtmp, "%sは冷たいまなざしをあなたに向けた！", Monnam(mtmp));
...
*/
    ...
    if (react >= 0) {
        ...
        pline_mon(mtmp, "%s looks %s%s.", Monnam(mtmp),
              ...
              reactions[react]);
#else
        pline_mon(mtmp, "%sは%s%sようだ．", Monnam(mtmp),
              !rn2(3) ? "" : already ? "かなり"
                                     : (!rn2(2) ? "少し" : "いくらか"),
              reactions[react]);
#endif
...
#else
        pline_mon(mon, "%sは%s頭が痛いふりをした．",
                  Monnam(mon), mon->mcan ? "ひどく" : "");
#endif
        return 0;
    }
    if (unresponsive()) {
        ...
        pline_mon(mon, "%s seems dismayed at your lack of response.",
                  Monnam(mon));
#else
        pline_mon(mon, "%sは返事がないので気が萎えたようだ．",
                  Monnam(mon));
#endif
        return 0;
    }
...
*/
            pline_mon(mon, "%sはため息をついたようだ．", Monnam(mon));
...
*/
            You("いつまでも%sを覚えてるだろう．．．", noit_mon_nam(mon));
            (void) adjattrib(A_WIS, 1, TRUE);
            exercise(A_WIS, TRUE);
            disp.botl = TRUE;
            break;
...
*/
    ...
    if (mon->mtame) { /* don't charge */
        ...
        pline_mon(mon, "%s tries to take your gold, but fails...",
                  noit_Monnam(mon));
#else
        pline_mon(mon, "%sは金を取ろうとしたが，失敗した．．．",
                  noit_Monnam(mon));
#endif
    } else {
        long cost;
...
#else
            pline_mon(mon, "%sはサービス料として%ld%s奪い取った！",
                      noit_Monnam(mon), cost, currency(cost));
#endif
            money2mon(mon, cost);
            disp.botl = TRUE;
...
#else
        Sprintf(hairbuf,
                flags.female ? "なんて綺麗な%sなんだ" : "兜を取ったらなかなかイカス%sじゃない",
                body_part(HAIR));
#endif
        SetVoice(mon, 0, 80, 0);
#if 0 /*JP:T*/
...
#else
            pline_mon(mtmp, "%sは%s%sを食らった！", Monnam(mtmp),
                  /* temporary? hack for sequencing issue:  "your acid"
                     looks strange coming immediately after player has
                     been told that hero has reverted to normal form */
...
*/
                pline_mon(mtmp, "%sは影響を受けない．", Monnam(mtmp));
                tmp = 0;
            }
        } else
            tmp = 0;
...
*/
                        ...
                        pline_mon(mtmp, "%s is frozen by your gaze!",
                                  Monnam(mtmp));
#else
                        pline_mon(mtmp, "%sはあなたのにらみで動けなくなった！",
                                  Monnam(mtmp));
#endif
...
*/
                pline_mon(mtmp, "%sは動けない．", Monnam(mtmp));
                paralyze_monst(mtmp, tmp);
                return M_ATTK_AGR_DONE;
            }
            return M_ATTK_HIT;
...
*/
                pline_mon(mtmp, "%sは冷えた．", Monnam(mtmp));
...
*/
            pline_mon(mtmp, "%sは突然凍りづけになった！", Monnam(mtmp));
            u.mh += (tmp + rn2(2)) / 2;
            if (u.mhmax < u.mh)
                u.mhmax = u.mh;
...
#else
                pline_mon(mtmp, "%sは暖かくなった．", Monnam(mtmp));
#endif
...
#else
            pline_mon(mtmp, "%sは突然熱くなった！", Monnam(mtmp));
#endif
            break;
        case AD_ELEC:
...
#else
                pline_mon(mtmp, "%sはちょっとピリピリした．", Monnam(mtmp));
#endif
            ...
            pline_mon(mtmp, "%s is jolted with your electricity!",
                      Monnam(mtmp));
#else
            pline_mon(mtmp, "%sは電気ショックをうけた！", Monnam(mtmp));
#endif
            break;
        default:
            tmp = 0;
