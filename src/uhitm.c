...
#else
                pline("%s%sが現れた！", mtmp->mtame ? "手なづけられた" : "野生の",
...
*/
        ...
        pline("%sの%sはあなたの一撃で粉々になった！", 
              Monnam(mon), canseemon(mon) ? xname(monwep) : "武器");
#endif
...
#define useup_eggs(o)                    \
    ...
    case EGG: {
        ...
        if (ismnum(obj->corpsenm)
            ...
            You("%sに%s%s卵を投げつけた．", mon_nam(mon), eggp, *eggp ? "の" : "");
#endif
...
*/
    You("%sに突撃した%s", mon_nam(mon), canseemon(mon) ? exclam(hmd->dmg) : "．");
...
#else
            char temp[BUFSZ];
            Strcpy(temp, hmd->saved_oname);
            (void) strNsubst(temp, "%", "%%", 0);
            Snprintf(silverobjbuf, sizeof(silverobjbuf), "%%sは%s%sで焼かれた！",
                     strstri(hmd->saved_oname, "銀") ? "" : "銀の",
                     temp);
#endif
            fmt = silverobjbuf;
        } else
...
#else
        Your("%sはもう毒が塗られていない．", hmd.saved_oname);
#endif
...
#else
            You("%sをうっとりさせた．%sはよろこんで持ち物%sをさしだした．",
                mon_nam(mdef), upstart(strcpy(heshe, mhe(mdef))),
                !gold ? "" : "のほとんど");
#endif
        else
...
#else
            pline("%sはバラバラにな%s！", Monnam(mdef),
                    !mlifesaver(mdef) ? "った" : "りはじめた");
#endif
        ...
        if (completelyrusts(pd)) { /* PM_IRON_GOLEM */
            if (gv.vis && canseemon(mdef))
                ...
                pline_mon(mdef, "%s %s to pieces!", Monnam(mdef),
                      !mlifesaver(mdef) ? "falls" : "starts to fall");
#else
                pline_mon(mdef, "%sはバラバラにな%s！", Monnam(mdef),
                      !mlifesaver(mdef) ? "った" : "りはじめた");
#endif
...
*/
                pline_mon(mdef, "%sは弱くなった！", Monnam(mdef));
            if (mdef->mhpmax - mhm->damage > (int) mdef->m_lev) {
                mdef->mhpmax -= mhm->damage;
            } else {
                /* limit floor of mhpmax reduction to current m_lev + 1;
...
#else
                pline("%sは完全に%s！", Monnam(mdef),
                      !mlifesaver(mdef) ? "燃え尽きた"
                                        : "炎に包まれた");
#endif
            else
#if 0 /*JP:T*/
            ...
            if (gv.vis && canseemon(mdef))
                ...
                pline_mon(mdef, "%s %s!", Monnam(mdef),
                      !mlifesaver(mdef) ? "burns completely"
                                        : "is totally engulfed in flames");
#else
                pline_mon(mdef, "%sは完全に%s！", Monnam(mdef),
                      !mlifesaver(mdef) ? "燃え尽きた"
                                        : "炎に包まれた");
#endif
...
*/
                pline_The("%sは炎で燃えないようだ！", mon_nam(mdef));
            shieldeff(mdef->mx, mdef->my);
            golemeffects(mdef, AD_FIRE, mhm->damage);
            mhm->damage = 0;
        }
...
*/
            pline_mon(mdef, "%sは氷で覆われた！", Monnam(mdef));
        if (resists_cold(mdef) || defended(mdef, AD_COLD)) {
            if (gv.vis && canseemon(mdef))
/*JP
                pline_The("frost doesn't seem to chill %s!", mon_nam(mdef));
*/
                pline_The("氷は%sを凍らせることができないようだ！", mon_nam(mdef));
            shieldeff(mdef->mx, mdef->my);
            golemeffects(mdef, AD_COLD, mhm->damage);
            mhm->damage = 0;
        }
...
*/
            pline_mon(mdef, "%sは衝撃をくらった！", Monnam(mdef));
        if (resists_elec(mdef) || defended(mdef, AD_ELEC)) {
            if (gv.vis && canseemon(mdef))
/*JP
                pline_The("zap doesn't shock %s!", mon_nam(mdef));
*/
                pline_The("衝撃は%sに影響を与えない！", mon_nam(mdef));
            shieldeff(mdef->mx, mdef->my);
            golemeffects(mdef, AD_ELEC, mhm->damage);
            mhm->damage = 0;
        }
...
*/
            pline_mon(mdef, "%sは%sにつつまれた！", Monnam(mdef), hliquid("酸"));
/*JP
...
*/
            pline("%sは%sからお金を盗んだ．", buf, mon_nam(mdef));
        }
...
*/
                pline("%sの攻撃で目が見えなくなった！", Monnam(magr));
...
*/
                    pline_mon(magr, "%sはくすくす笑った．", Monnam(magr));
                }
            }
...
*/
                    pline_mon(mdef, "%sは破壊された！", Monnam(mdef));
                }
                mondied(mdef);
                if (!DEADMONSTER(mdef)) {
                    mhm->hitflags = M_ATTK_MISS;
...
*/
            Your("%sは毒されている！", mpoisons_subj(magr, mattk));
            if (resists_poison(mdef)) {
/*JP
                pline_The("poison doesn't seem to affect %s.", mon_nam(mdef));
*/
                pline("毒は%sに効かなかったようだ．", mon_nam(mdef));
            } else {
                if (!rn2(10)) {
/*JP
...
*/
                pline_mon(mdef, "%sは傷ついたようには見えない．", Monnam(mdef));
            /* Not clear what to do for green slimes */
            mhm->damage = 0;
                ...
                if (flags.verbose) {
                    if (coil && !tailmiss)
                        ...
                        You("%sの%sに触れた．", mon_nam(mdef),
                            tailmiss ? "尻尾" : mbodypart(mdef, LEG));
#endif
                }
            }
        } else
...
*/
                    urgent_pline("あなたは%sに絡みつかれて溺れた．．．", Monnam(magr));
...
#else
                    Sprintf(svk.killer.name, "%sの%sに絡みつかれて",
                            moat ? "堀" : "池",
...
*/
                dynamic_multi_reason(magr, "麻痺", FALSE);
...
*/
                pline("%sは%sに眠らされた．", buf, mon_nam(magr));
            }
            mdef->mstrategy &= ~STRAT_WAITFORU;
            slept_monst(mdef);
        }
...
*/
                pline_mon(mdef, "%sは動作がのろくなった．", Monnam(mdef));
        }
    }
}
...
*/
                pline_mon(mdef, "%sは混乱しているように見える．", Monnam(mdef));
            mdef->mconf = 1;
            mdef->mstrategy &= ~STRAT_WAITFORU;
        }
    }
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        pline_mon(magr, "%s reaches out, and your body shrivels.",
                  Monnam(magr));
#else
        pline_mon(magr, "%sは腕を伸ばした，あなたの体はしなびた．",
                  Monnam(magr));
#endif
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        pline_mon(magr, "%s reaches out, and you feel fever and chills.",
                  Monnam(magr));
#else
        pline_mon(magr, "%sは腕をのばした．あなたは悪寒を感じた．",
                  Monnam(magr));
#endif
        (void) diseasemu(pa);
        /* plus the normal damage */
...
*/
        pline_mon(magr, "%sは死の腕をのばした．", Monnam(magr));
        if (is_undead(pd)) {
        ...
        if (!magr->mcan && haseyes(pd) && mdef->mcansee) {
            if (gv.vis && canseemon(mdef))
                ...
                pline_mon(mdef, "%s looks %sconfused.", Monnam(mdef),
                      mdef->mconf ? "more " : "");
#else
                pline_mon(mdef, "%sは%s混乱しているように見える．", Monnam(mdef),
                      mdef->mconf ? "ますます" : "");
#endif
            mdef->mconf = 1;
            mdef->mstrategy &= ~STRAT_WAITFORU;
...
*/
                    pline_mon(magr, "%sにつかまえられている！", Monnam(magr));
                    mhm->hitflags |= M_ATTK_HIT;
                }
            } else if (u.ustuck == magr) {
                exercise(A_STR, FALSE);
...
#else
                You("%sている．",
                    (pa == &mons[PM_ROPE_GOLEM]) ? "首を絞められ" : "押しつぶされ");
#endif
            }
...
*/
                        pline("%sの攻撃によってあなたは分裂した！", mon_nam(magr));
                }
...
*/
                    You_hear("%sがゴホッゴホッという音を聞いた！", mon_nam(magr));
            } else {
...
#else
                    You_hear("シューッという声を聞いた．"); /* You_hear() deals with Deaf */
#endif
#if 0 /*JP:T*/
                    pline("%s appears to be blowing you a kiss...",
                          Monnam(magr));
#else
                    pline("%sあなたにキスを飛ばしているようだ．．．",
                          Monnam(magr));
#endif
                } else if (!Deaf) {
/*JP
...
*/
                    You_hear("%sがシューッという声を聞いた！", mon_nam(magr));
                } else if (!Blind) {
/*JP
                    pline("%s seems to grimace.", Monnam(magr));
*/
                    pline("%sは顔をしかめているようだ．", Monnam(magr));
                }
                /*
    ...
    if (magr == &gy.youmonst) {
        ...
        if (!(uwep && (uwep->oclass == WEAPON_CLASS || is_weptool(uwep)))
            ...
            pline_mon(magr, "%s hits!  (I hope you don't mind.)",
                      Monnam(magr));
#else
            pline_mon(magr, "%sの攻撃は命中した！(気にしないように．)",
                      Monnam(magr));
#endif
            if (Upolyd) {
                u.mh += rnd(7);
    ...
    if (magr == &gy.youmonst) {
        ...
        if (canseemon(mdef))
            ...
            pline_mon(mdef, "%s %s for a moment.", Monnam(mdef),
                  makeplural(stagger(pd, "stagger")));
#else
            pline_mon(mdef, "%sは一瞬%s．", Monnam(mdef),
                  jpast(stagger(pd, "よろめく")));
#endif
        mdef->mstun = 1;
        mhitm_ad_phys(magr, mattk, mdef, mhm);
...
*/
            ...
            pline_mon(magr, "%s nuzzles against your %s %s!", Monnam(magr),
                  sidestr, leg);
#else
            pline_mon(magr, "%sはあなたの%s%sに鼻をすりよせた！", Monnam(magr),
                  sidestr, leg);
#endif
            mhm->damage = 0;
        } else {
...
#else
                pline("%sはあなたを%sしようとした，しかしあなたは%sようだ．",
                      Adjmonnam(magr, "地味な"),
                      flags.female ? "魅了" : "誘惑",
...
#else
    You("%s%sを%s！",
        mon_nam(mdef), u_digest ? "全体" : "",
        u_digest ? "飲み込んだ" : u_enfold ? "捕まえた" : "巻き込んだ");
#endif
    nh_delay_output();
    nh_delay_output();
}
...
gulpum(struct monst *mdef, struct attack *mattk)
{
    ...
    const char *expel_verb = u_digest ? "regurgitate"
                             : u_enfold ? "release"
                               : "expel";
#else
    const char *expel_verb = u_digest ? "吐き戻"
                             : u_enfold ? "解放"
                               : "排出";
#endif
...
#else
            You("%s，%sした．",
                u_digest ? "巻き込んで" : u_enfold ? "捕まえて" : "飲み込んで",
                expel_verb);
#endif
            if (canspotmon(mdef)) {
...
*/
            You("%sを%s込んだ．", u_digest ? "飲み" : "巻き", mon_nam(mdef));
#if 0 /*JP:T*/
            Sprintf(kbuf, "%s %s%s",
                    u_digest ? "swallowing"
                    : u_enfold ? "enclosing"
...
#else
                    Sprintf(svk.killer.name, "愚かにも%sを食べようとして",
                            pmname(pd, Mgender(mdef)));
...
*/
                    ...
                    pline("%sは%s！", Monnam(mdef),
                          enfolds(gy.youmonst.data) ? "押しつぶされた"
                            : "瓦礫で痛めつけられた");
#endif
                }
                break;
...
#else
            You("%sを%sした！", mon_nam(mdef), expel_verb);
#endif
...
*/
                        dynamic_multi_reason(mon, "硬直", TRUE);
                        gn.nomovemsg = 0;
                    }
                } else {
#if 0 /*JP:T*/
...
*/
                dynamic_multi_reason(mon, "硬直", FALSE);
                exercise(A_DEX, FALSE);
            }
            break;
...
#else
                Snprintf(fmtbuf, sizeof fmtbuf, "%sは実際には%%sだった！",
                         defsyms[sym].explanation);
#endif
