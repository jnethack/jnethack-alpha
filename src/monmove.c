...
msg_mon_movement(struct monst *mtmp, coordxy omx, coordxy omy)
{
    if (a11y.mon_movement && canspotmon(mtmp) && mtmp->mspotted) {
        ...
        pline_xy(nix, niy, "%s %s%s.", Monnam(mtmp),
                 ...
                 : " in the distance");
#else
        pline_xy(nix, niy, "%sは%s%s．", Monnam(mtmp),
                 n2u ? "あなたの隣に"
                 : (close && closer) ? "より近くに"
                 : (close && !closer) ? "遠くへ"
                 : "遠くで",
                 jpast(locomotion(mtmp->data, "動く")));
#endif
    }
}
...
*/
            pline_mon(mtmp, "ちゅどーん！ドアが爆発するのを見た．");
        else if (!Deaf)
#if 0 /*JP:T*/
            You_hear("a %s explosion.",
                     (mdistu(mtmp) > 7 * 7) ? "distant" : "nearby");
#else
            You_hear("%sくで爆発する音を聞いた．",
                     (mdistu(mtmp) > 7 * 7) ? "遠" : "近");
#endif
    }
...
*/
                pline_mon(mtmp, "%sはおびえて逃げ出した！", Monnam(mtmp));
            }
        }
...
*/
        pline_mon(mtmp, "%sは精神を集中している．", Monnam(mtmp));
...
*/
        pline_mon(mtmp, "%sはあなたから逃げられない！", Monnam(mtmp));
        return TRUE;
    }
    return FALSE;
}
...
#else
                    pline_mon(mtmp, "%sは扉の下から%s出た．", YMonnam(mtmp),
                              (ptr == &mons[PM_FOG_CLOUD]
...
*/
                            pline_mon(mtmp, "%sは扉を開けた．", Monnam(mtmp));
                        } else if (canseeit) {
/*JP
                            You_see("a door open.");
*/
