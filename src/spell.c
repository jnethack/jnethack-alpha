...
#else
        You("%sの行を何度も繰り返して読んでいたことに気づいた．",
            spellbook == svc.context.spbook.book ? "次" : "最初");
#endif
    }
    return gone;
}
...
*/
        Snprintf(qbuf, sizeof qbuf, "どの魔法を唱える？[%s *?]", lets);
