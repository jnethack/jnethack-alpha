...
*/
        pline("“S‹…‚Í‚ ‚È‚½‚Ì%s‚Ìã‚É—Ž‚¿‚½D", body_part(HEAD));
        if (uarmh) {
            if (is_metallic(uarmh)) {
/*JP
                pline("Fortunately, you are wearing a hard helmet.");
*/
                pline("K‰^‚É‚àC‚ ‚È‚½‚ÍŒÅ‚¢Š•‚ðg‚É‚Â‚¯‚Ä‚¢‚½D");
                dmg = 3;
            } else if (flags.verbose)
/*JP
                pline("%s does not protect you.", Yname2(uarmh));
*/
                Your("%s‚Å‚ÍŽç‚ê‚È‚¢D", xname(uarmh));
        }
#if 0 /*JP*/
        losehp(Maybe_Half_Phys(dmg), "crunched in the head by an iron ball",
               NO_KILLER_PREFIX);
#else
        losehp(Maybe_Half_Phys(dmg), "“S‹…‚Å“ª‚ð‘Å‚Á‚Ä", KILLED_BY);
#endif
    }
}
...
*/
        static const char *pullmsg = "“S‹…‚Í%s‚©‚ç‚ ‚È‚½‚ðˆø‚Á‚Ï‚èo‚µ‚½I";
        struct trap *t;
        long side;
        ...
        if ((otmp != uball) && (rnd(capacity) <= (int) otmp->owt)) {
            if (canletgo(otmp, "")) {
                ...
                You("drop %s and %s %s down the stairs with you.",
                    ...
                    otense(otmp, "fall"));
#else
                You("%s‚ð—Ž‚Æ‚µC‚»‚ê‚Í‚ ‚È‚½‚Æˆê‚ÉŠK’i‚ð—Ž‚¿‚Ä‚¢‚Á‚½D",
                    yname(otmp));
#endif
                dropx(otmp);
...
*/
            You("“S‹…‚É‚æ‚Á‚ÄŠK’i‚ð‚±‚ë‚ª‚è—Ž‚¿‚½I");
            losehp(Maybe_Half_Phys(rnd(6)),
/*JP
                   "dragged downstairs by an iron ball", NO_KILLER_PREFIX);
*/
                   "“S‹…‚É‚æ‚èŠK’i‚ð‚±‚ë‚ª‚è—Ž‚¿‚Ä", KILLED_BY);
            litter();
        }
    } else {
        if (rn2(2)) {
