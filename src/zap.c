...
*/
static const char are_blinded_by_the_flash[] = "まばゆい光で目が見えなくなった！";
...
#else
                pline("%s%sは突然%s%s%s!", owner, corpse,
                      different_type ? an(mon_pmname(mtmp2)) : "",
                      different_type ? "として" : "",
                      nonliving(mtmp2->data) ? "動き出した" : "生命を帯びた");
#endif
...
#else
        You_feel("恐怖し%sくらくらした．", Stunned ? "さらに" : "");
#endif
...
#else
        Strcpy(svk.killer.name, "自分が撃った死の光線によって");
...
#else
    static const char
        writing_vanishes[] = "いくつかの文字が%sの頭から消えた！",
        your[] = "あなた"; /* should be extern */
#endif
...
#else
                    pline("あなたの下で%sがうずを巻いた．",
                          is_ice(x,y) ? "霜" : "ほこり");
#endif
                } else {
                    ttmp->tseen = 1;                    newsym(x, y);
/*JP
    ...
    const char *force)  /* usually either "." or "!" via exclam() */
{
    ...
    pline("%sは%sに命中した%s", str,
          verbosely ? mon_nam(mtmp) : "それ", force);
#endif
}
...
#else
    pline("%sの%sへの攻撃ははずれた．", str,
          ((cansee(gb.bhitpos.x, gb.bhitpos.y) || canspotmon(mtmp))
           && flags.verbose) ? mon_nam(mtmp) : "それ");
#endif
}
...
#else
        const char *verb = (abstyp < 10) /* wand */
                           ? ((otmp && otmp->oclass == TOOL_CLASS) ? "演奏した"
                              : "振った")
                           : (abstyp < 20) ? "唱えた"
                             : (abstyp < 30) ? "はいた"
...
#else
            Sprintf(kbuf, "自分自身に%s%s", verb, fltxt);
#endif
        }
        ...
        if (obj->oclass == SCROLL_CLASS || obj->oclass == SPBOOK_CLASS
            ...
            for (i = scrquan; i > 0L; i--)
                if (!rn2(3))
                    delquan++;
            if (delquan) {
#if 1 /*JP*//*objは途中で解放されることがあるのでここで計算*/
                const char *numstr;
                long quanbak = obj->quan;
                obj->quan = delquan;
                numstr = numeral(obj);
                obj->quan = quanbak;
#endif
...
#else
            pline("%sは%sをひきさいた%s", flash_str(fltyp, FALSE),
                  mon_nam(u.ustuck), exclam(tmp));
#endif
...
#else
                pline_dir(xytodir(-dx, -dy), "%sはあなたに命中した！",
...
*/
                pline("%sはあなたのそばをかすめた！", flash_str(fltyp, FALSE));
...
*/
                    pline_The("%sは反射した！", flash_str(fltyp, FALSE));
            }
    ...
    if (shopdamage)
        ...
        pay_for_damage(damgtype == ZT_FIRE ? "burn away"
                       ...
                       FALSE);
#else
        pay_for_damage(damgtype == ZT_FIRE ? "燃やす"
                       : damgtype == ZT_COLD ? "粉々にする"
                         /* "damage" indicates wall rather than door */
                         : damgtype == ZT_ACID ? "傷つける"
                           : damgtype == ZT_DEATH ? "粉砕する"
                             : "破壊する",
                       FALSE);
#endif
...
#else
                    Norep("%sは少し%sがまだしっかりしている．",
                          defsyms[S_bars].explanation,
                          (damgtype == ZT_ACID) ? "痛んだ" : "溶けた");
#endif
...
#else
                    Norep("%sは%s．", defsyms[S_bars].explanation,
                          (damgtype == ZT_ACID) ? "ぼろぼろになった" : "溶けた");
#endif
                dissolve_bars(x, y);
        ...
        if (obj->otyp == SPE_BOOK_OF_THE_DEAD) {
            ...
            if (u_carry ? !Blind : vis) {
                ...
                pline("%sは奇妙に%s輝いたが何も変化しなかった．",
                      u_carry ? xname(obj) : distant_name(obj, xname),
                      hcolor_adv("暗褐色の"));
#endif
            }
            break;
...
*/
    ...
    (void) hold_another_object(otmp, oopsbuf, xname(otmp), 
                               (const char *) 0);
#endif
