...
static const char none[] = "(none)", randomrole[] = "random",
    ...
    static const char none[] = "(なし)", randomrole[] = "ランダム",
                      to_be_done[] = "(未設定)",
                      defopt[] = "default", defopt_disp[] = "デフォルト", defbrief[] = "def";
#endif
...
*/
static const char n_currently_set[] = "(%d個設定中)";
...
*/
    ...
    if (req == get_val) {
        ...
        Sprintf(opts, "%s", (gp.preferred_pet == 'c') ? "猫"
                           : (gp.preferred_pet == 'd') ? "犬"
                             : (gp.preferred_pet == 'h') ? "馬"
                               : (gp.preferred_pet == 'n') ? "なし"
                                 : "ランダム");
#endif
        return optn_ok;
    }
        ...
        if (op == empty_optstr) {
            ...
            if (flags.menu_style == MENU_TRADITIONAL
                ...
                Sprintf(qbuf, "New %s: [%s am] (%s)", allopt[optidx].name,
                        ocl, *tbuf ? tbuf : "all");
#else
                Sprintf(qbuf, "新しい%s: [%s am] (%s)", allopt[optidx].name,
                        ocl, *tbuf ? tbuf : "全て");
#endif
                abuf[0] = '\0';
                getlin(qbuf, abuf);
            ...
            if (use_menu) {
                ...
                (void) choose_classes_menu("Autopickup what?",
                                           1, TRUE, ocl, tbuf);
#else
                (void) choose_classes_menu("どれを自動拾いに設定する？",
                                           1, TRUE, ocl, tbuf);
#endif
                op = tbuf;
            }
    ...
    char *opts, char *op)
{
    ...
    if (req == get_val || req == get_cnf_val) {
        ...
        if (req == get_val)
            Sprintf(opts, "%s入力",
                iflags.wc_player_selection ? "プロンプト" : "ダイアログ");
        else
#endif
        Sprintf(opts, "%s",
...
pfxfn_font(int optidx, int req, boolean negated, char *opts, char *op)
{
    ...
    if (req == get_val || req == get_cnf_val) {
#if 1 /*JP*/
        const char *defopt = (req == get_cnf_val) ? "default" : defopt_disp;
#endif
        if (optidx == opt_font_map) {
            Sprintf(opts, "%s",
...
handler_align_misc(int optidx)
{
    ...
    Sprintf(abuf, "%sウィンドウのマップに対する表示位置を選択してください：",
            (optidx == opt_align_message) ? "メッセージ" : "状態");
#endif
...
#else /*「選んでください」は呼び出し先で特別処理が入るので使わない*/
    int tmp
        = query_attr("ペットをハイライトする属性", iflags.wc2_petattr);
#endif
...
#else
        Strcpy(apebuf, (opt_idx == 1) ? "自動拾い例外一覧"
                                      : "削除する自動拾い例外を選択");
#endif
...
#else
    config_error_add("%sオプションは否定%sできない．", optname,
...
#else
    config_error_add("%sオプションが複数回指定されています：%s%s",
                     (allopt[optidx].opttyp == CompOpt) ? "複合"
                                                        : "真偽値",
                     allopt[optidx].name, buf);
#endif
#endif /* ?MACOS9 */
...
*/
                Sprintf(buf, "%sに何を設定する？", allopt[k].name);
                getlin(buf, abuf);
                if (abuf[0] != '\033') { /* ESC */
...
*/
    add_menu_heading(tmpwin, "真偽オプション (選択すると値が切り替わります)：");
    any.a_int = 0;
...
*/
                    Sprintf(buf, "%sに何を設定する？", allopt[opt_indx].name);
                    abuf[0] = '\0';
                    getlin(buf, abuf);
                    if (abuf[0] == '\033')
                        continue;
...
static const char *const opt_epilog[] = {
    ...
    "OPTIONSではなく独自の指示子を使用します．",
    "詳細はJNetHackの\"Guidebook\"を参照してください．",
#endif
    (char *) 0
};
...
*/
             "オプションは%sの中でOPTIONS=<options>と設定します", get_configfile());
