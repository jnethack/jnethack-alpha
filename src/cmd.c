...
*/
const char unavailcmd[] = "'%s'コマンドは使えない．";
...
#else
        Sprintf(buf, "自動補完されないコマンドを%sように切り替える",
                menumode ? "含める" : "除外する");
#endif
        any.a_int = 1;
...
#else /*足りなくなるかもしれないので増やしておく*/
    char cbuf[BUFSZ], prompt[BUFSZ], fmtstr[20];
#endif
        ...
        for (efp = extcmdlist; efp->ef_txt; efp++) {
            ...
            if (!matchlevel || !strncmp(efp->ef_txt, cbuf, matchlevel)) {
                ...
                if ((len = (int) strlen(efp->ef_desc)) > biggest)
                    biggest = len;
#else
                if ((len = displen(efp->ef_desc)) > biggestdisp) {
                    biggestdisp = len;
...
*/
        const char *oldmode = !wizard ? "通常プレイ" : "デバッグモード";
...
#else
    { '@',    "autopickup", "'autopickup'オプションのオン/オフを切り替える",
#endif
...
#else
    { M('n'), "name", "callと同じ; 怪物，特定の物，または物の種類に名前を付ける",
#endif
...
#else
    { '\0',   "optionsfull", "全てのオプション設定を表示し，必要に応じて変更する",
#endif
...
#else
    { 'R',    "remove", "アクセサリをはずす (指輪，魔除けなど)",
#endif
              doremring, 0, NULL },
#if 0 /*JP:T*/
    { C('a'), "repeat", "repeat a previous command",
#else
    { C('a'), "repeat", "前回のコマンドを繰り返す",
#endif
...
#else
    { 's',    "search", "罠や隠し扉を探す",
              dosearch, IFBURIED | CMD_M_PREFIX, "探す" },
#endif
#if 0 /*JP:T*/
    { '*',    "seeall", "show all equipment in use",
#else
    { '*',    "seeall", "使用している全ての装備類を表示する",
#endif
...
#else
    { C('z'), "suspend", "ゲームをバックグラウンドに送る（戻るには'fg'）",
#endif
...
#else
    { 'A',    "takeoffall", "全ての防具を外す",
#endif
              doddoremarm, 0, NULL },
#if 0 /*JP:T*/
    { C('t'), "teleport", "teleport around the level",
#else
    { C('t'), "teleport", "同じ階の中で瞬間移動する",
#endif
...
#else
    { 'V',    "versionshort", "バージョンとプログラムのビルド日時を表示する",
#endif
...
#else
    { '.',    "wait", "一歩分何もしない",
              donull, IFBURIED | CMD_M_PREFIX, "休憩する" },
#endif
#if 0 /*JP:T*/
...
#else
    { NHKF_ESC, "現在のプロンプトや保留中の接頭辞を中断する", FALSE },
#endif
    { NHKF_COUNT,
...
#else
            (void) strsubst(key2cmdbuf, "接頭辞:",
                     /* relies on implicit concatenation of literal strings */
                            "移動接頭辞:"
                            " 自動拾いと攻撃なしで移動"
                            "\n"
                            "非移動接頭辞:"); /* and rest of buf */
#endif
...
*/
    Sprintf(buf, "%7s %s", "", "    現在の完全なキー割り当て一覧");
    putstr(datawin, 0, buf);
    for (extcmd = extcmdlist; extcmd->ef_txt; ++extcmd)
