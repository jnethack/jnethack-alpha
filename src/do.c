...
{
#if 1 /*JP*/
    extern const char *set_you[2]; /* trap.c */
#endif
    struct trap *t;
    struct monst *mtmp;
    struct obj *otmp;
...
#else
                pline_The("岩は%s%s．",
                          (ttyp == TRAPDOOR && !tseen) ? "罠を起動して，" : "",
                          (ttyp == TRAPDOOR) ? "落し扉を埋めた"
                          : (ttyp == HOLE) ? "穴を埋めた"
                            : "落し穴を埋めた");
#endif
            } else {
...
#else
            pline("%sは熱い地面に落ちたので加熱された．", xname(obj));
#endif
        }
...
#else
                pline("%sは熱で割れた！", xname(obj));
#endif
            } else {
/*JP
                You_hear("a shattering noise.");
*/
                You_hear("何かが割れる音を聞いた．");
            }
...
*/
            pline("流し台は一瞬%s輝いた．", hcolor_adv(NH_WHITE));
            break;
    ...
    if (obj == uwep && welded(uwep)) {
        ...
        if (*word) {
            ...
            Norep("You cannot %s %s welded to your %s.", word, something,
                  hand);
#else
            Norep("%sに貼り付いているものを%sことはできない．", hand, word);
#endif
        }
        return FALSE;
    }
...
*/
        n = query_category("どの種類のアイテムを置く？", gi.invent,
...
#else
        n = query_objlist("どれを置きますか？", &gi.invent,
                          (USE_INVLET | INVORDER_SORT | INCLUDE_VENOM),
                          &pick_list, PICK_ANY,
                          all_categories ? allow_all : allow_category);
#endif
...
#else
            You("%s%sことができない．",
                !u.uswallow ? "つかまえられていて" 
                : digests(u.ustuck->data) ? "飲み込まれていて"
                : "巻き込まれていて", updn);
#endif
            return TRUE;
        } else {
            ...
            if (flags.autodig && !svc.context.nopick
                ...
                You_cant("go down here%s.",
                         (trap && trap->ttyp == VIBRATING_SQUARE) ? " yet"
                                                                  : "");
#else
                You_cant("ここでは%s降りることができない．",
                         (trap && trap->ttyp == VIBRATING_SQUARE) ? "まだ"
                                                                  : "");
#endif
                return ECMD_OK;
            }
...
*/
                You("%sを転げ落ちた．", ga.at_ladder ? "はしご" : "階段");
                if (Punished) {
                    drag_down();
                    if (!welded(uball))
                        ballrelease(FALSE);
...
#else
        pline("ここは%sい．",
              svl.level.flags.temperature > 0 ? "熱" : "寒");
#endif
...
#else
        You("煙%sを感じた．．．",
              olfaction(gy.youmonst.data) ? "のにおい" : "");
#endif
}
...
#else
                    pline("%sは，%sが%sのにびっくりして，%sを落した！",
                          mon_nam(mcarry), cname,
                          canspotmon(mtmp) ? "生きかえった" : "消えた",
                          cname);
#endif
...
*/
                pline("%sは%sから逃れようともがいた！", mnam, yname(container));
...
#else
                pline("%sは袋の中で%sから逃れようともがいた！", mnam,
                      sackname);
#endif
...
*/
        pline("%sは%s状態ではない．", Monnam(u.usteed), for_what);
    } else {
