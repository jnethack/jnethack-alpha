...
*/
    ...
    if (!uarmg && obj->otyp == CORPSE && touch_petrifies(&mons[obj->corpsenm])
        ...
        Sprintf(svk.killer.name, "throwing %s bare-handed",
                killer_xname(obj));
#else
        Sprintf(svk.killer.name, "%sを素手で投げて",
                killer_xname(obj));
#endif
...
#else
        pline("%sは%sに%s．", Doname2(obj), surf, verb);
#endif
    }
...
#else
            why = IS_TREE(ltyp) ? "木にぶちあたって"
                  : IS_OBSTRUCTED(ltyp) ? "壁にぶちあたって"
                    : odoor_diag ? "扉の端にぶちあたって"
                      : "閉じている扉にぶちあたって";
#endif
            if (odoor_diag)
/*JP
...
#else
                You("%sの真上を通過した．", trapname(ttmp->ttyp, FALSE));
#endif
        }
    }
...
*/
        ...
        You("%sで固定されている．",
            ...
            : (u.utraptype == TT_LAVA) ? hliquid("溶岩")
              : (u.utraptype == TT_INFLOOR) ? surface(u.ux, u.uy)
...
#else
            Strcpy(svk.killer.name, "初等物理により");
#endif
/*JP
            You("turn to stone.");
*/
...
*/
            pline_The("あなたは銀で焼かれた！");
        if (harmless)
/*JP
            hit(thesimpleoname(obj), &gy.youmonst, " but doesn't hurt.");
*/
            hit(thesimpleoname(obj), &gy.youmonst, "けれども，なんともなかった．");
        ...
        if (is_ammo(obj)) {
            if (ammo_and_launcher(obj, uwep)) {
                ...
                pline("You aren't wielding %s, so you throw your %s by %s.",
                      ...
                      body_part(HAND));
#else
                pline("あなたは%sを装備していないので，%sを%sで投げた．",
...
*/
            pline("%sは%sを掴んだ．", Some_Monnam(mon), xname(obj));
...
#else
                pline("%sは%sをあなたに返した．", Some_Monnam(mon),
                      xname(obj));
#endif
                if (!next2u)
...
*/
    ...
    if (u.uswallow) {
        ...
        pline_The("金貨は%s%sの中へ消えていった．", swallower,
                  digests(u.ustuck->data) ? "の体" : "");
#endif
...
*/
        pline_The("金貨は%sに命中した．", surface(gb.bhitpos.x, gb.bhitpos.y));
    place_object(obj, gb.bhitpos.x, gb.bhitpos.y);
    if (*u.ushops)
