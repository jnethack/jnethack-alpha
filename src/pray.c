...
*/
static const char *const Moloch = "モーロック";
...
*/
            pline("%sは%sやわらかく輝いた．", xname(otmp), hcolor_adv(NH_AMBER));
            set_bknown(otmp, 1);
        }
        uncurse(otmp);
        break;
...
*/
    Sprintf(svk.killer.name, "%sの怒りに触れ", align_gname(resp_god));
    done(DIED);
}
    ...
    case 3:
        ...
        pline("「汝%s，%sよ．」",
              (ugod_is_angry() && resp_god == u.ualign.type)
                  ? "その道から踏み出ておる"
                  : "傲慢なり",
              gy.youmonst.data->mlet == S_HUMAN ? "定命のもの" : "生物");
#endif
        SetVoice((struct monst *) 0, 0, 80, voice_deity);
/*JP
...
#else
        verbalize("汝，我が栄光のため%s者として選ばれん！", what);
#endif
        ...
        if ((spe_let = force_learn_spell(otmp->otyp)) != '\0') {
            ...
            if (spe_knowledge == spe_Unknown) /* prior to learning */
                ...
                pline("Divine knowledge of %s fills your mind!  Spell '%c'.",
                      spe_name, spe_let);
#else
                pline("%sに関する神聖な知識があなたの心を満たした！呪文'%c'．",
                      spe_name, spe_let);
#endif
            else
#if 0 /*JP:T*/
                Your("knowledge of spell '%c' - %s is %s.",
                     ...
                     (spe_knowledge == spe_Forgotten) ? "restored"
                                                      : "refreshed");
#else
                pline("呪文'%c'の知識 - %sが%sされた．",
                     spe_let, spe_name,
                     (spe_knowledge == spe_Forgotten) ? "復元"
                                                      : "刷新");
#endif
        }
            ...
            if (!u.uevent.uopened_dbridge && !u.uevent.gehennom_entered) {
                if (u.uevent.uheard_tune < 1) {
                    ...
                    verbalize("Hark, %s!", is_human(gy.youmonst.data)
                                               ...
                                               : "creature");
#else
                    verbalize("%sよ，聞け！", is_human(gy.youmonst.data)
                                               ? "定命の者"
                                               : "生物");
#endif
                    SetVoice((struct monst *) 0, 0, 80, voice_deity);
...
#else
    Sprintf(gvbuf, "定命の者よ！おまえは我が%sを汚すのか！",
            highaltar ? "神聖なる寺院" : "祭壇");
#endif
...
#else
        pline("%sは肩をすくめ，%sに対する優勢を維持した．", Moloch, u_gname());
#endif
/*JP
...
*/
        ...
        svk.killer.format = KILLED_BY;
#else
        Sprintf(svk.killer.name, "冷淡な%s", Moloch);
...
#else
        You("%sと%s間の争いを感じた．", u_gname(), a_gname());
#endif
        if (rn2(8 + u.ulevel) > 5) {
            struct monst *pri;
            boolean shrine;
...
#else
                pline("祭壇は%s輝いた．",
                          hcolor_adv((u.ualign.type == A_LAWFUL) ? NH_WHITE
                                 : u.ualign.type ? NH_BLACK
                                   : (const char *) "灰色の"));
#endif
...
*/
            pline("不幸にも，%sの力が減少したのを感じた．", u_gname());
            change_luck(-1);
...
*/
        pline_The("祭壇は%sの血で汚れている．", gu.urace.adj);
...
#else
            pline_The("%sの神に祈りをささげるという考えそのものがあなたには耐えがたい．",
                      gp.p_aligntyp ? "秩序" : "中立");
#endif
        return FALSE;
    }
...
*/
        You("%sに祈りを捧げはじめた．", align_gname(gp.p_aligntyp));
...
*/
        static const char forcesuccess[] = "無理矢理神に微笑んでもらいますか？";
...
#else
            pline("他に何も起こらなかった．"); /* not actually true... */
#endif
            return 1;
...
*/
    gm.multi_reason = "怪物を土に戻そうとしている時に";
