...
#else
        if (strstri(txt, "少し混乱") && Confusion)
            txt = strsubst(strcpy(buf, txt), "少し混乱", "さらに混乱");
#endif
        break;
    case 6:
...
#else
        if (strstri(txt, "思考") && Stunned)
            txt = strsubst(strcpy(buf, txt), "なったようだ", "なった");
#endif
        break;
    case 5:
...
#else
                urgent_pline(str, hcolor_adv(NH_BLUE));
#endif
            else
#if 0 /*JP:T*/
                urgent_pline("%s", str);
...
#else
                    urgent_pline(buf, hcolor_adv(NH_GREEN));
#endif
            } else {
#if 0 /*JP:T*/
...
*/
        Strcpy(svk.killer.name, "緑スライムになった");
    }
    dealloc_killer(kptr);
        ...
        if ((upp->intrinsic & TIMEOUT) && !(--upp->intrinsic & TIMEOUT)) {
            ...
            case STONED:
                if (kptr && kptr->name[0]) {
                    ...
                    Strcpy(svk.killer.name, kptr->name);
#else
                    Sprintf(svk.killer.name, "%sの攻撃で", kptr->name);
#endif
                } else {
#if 0 /*JP*/
                    svk.killer.format = NO_KILLER_PREFIX;
...
*/
    gn.nomovemsg = wakeup_msg ? "目を覚ました．" : You_can_move_again;
}
...
#else
            Sprintf(svk.killer.name, "%sの死体につまづいて",
...
#else
    boolean canseeit, menorah, need_newsym, need_invupdate, bytouch;
#endif
    coordxy x, y;
    char whose[BUFSZ];
