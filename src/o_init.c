...
#else
        add_menu_str(tmpwin, "注意：完全な名前順と種類内での名前順は");
        add_menu_str(tmpwin, "      単一クラス探索と等価ですが，");
        add_menu_str(tmpwin, "      将来の完全な探索では異なります．");
#endif
    }
/*JP
    end_menu(tmpwin, "Ordering of discoveries");
*/
    end_menu(tmpwin, "発見物の順番");
...
*/
                putstr(tmpwin, iflags.menu_headings.attr, "発見した物");
