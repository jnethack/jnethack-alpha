...
#error need VIRTUAL_TERMINAL_SEQUENCES
#endif
#endif
...
#if 1 /*JP*/
static void clear_console_cell(COORD pos)
{
    DWORD unused;
    static const char buf[1] = {CONSOLE_CLEAR_CHARACTER};
    ...
    for (pos.Y = 0; pos.Y < console.height; pos.Y++) {
        for (pos.X = 0; pos.X < console.width; pos.X++) {
            ...
            if (did_anything) {
                ...
                /* 漢字の1バイト目だった場合、
                   2バイト目をクリアして確実に更新する */
                if (front->iskanji == 1 && pos.X < console.width - 1) {
...
#if 1 /*JP*/
            if (back->iskanji == 1 && pos.X < console.width - 1) {
                back++;
                front++;
                pos.X++;
                *front = *back;
...
xputc2_core(const unsigned char *str)
{
    ...
    if (ret == 0) {
        /*ここに不正な文字が来るのは完全に壊れているのでpanic()を呼ぶ*/
        panic("xputc2_core: invalid char %s", str);
    }
...
xputc2(const unsigned char *str)
{
    ...
    /* wintty.c では 1 バイト毎に curx を加算するが、ここは
       charlen バイトたまってから呼び出されるので、len-1 文字分先に進んで
      しまっている。従って len-1 を引く。 */
    ttyDisplay->curx -= (len - 1);
    ...
    /* 実際に何バイトでも移動するのは 2 バイト分
       戻ってから 1 バイト足されるのでここでは 1 バイト分のみ */
    ttyDisplay->curx++;
    ...
    if (redirect_stdout || program_state.early_options)
        ...
        if (ttyDisplay) {
            set_console_cursor(ttyDisplay->curx, ttyDisplay->cury);
            {
                char *str = buf;
                while(*str){
                    jbuffer(*(str++), (void (__cdecl *)(unsigned int))xputc_core, (void (__cdecl *)(unsigned char *))xputc2_core);
                }
            }
        } else {
            printf("%s", buf);
    ...
    int portdebug)
{
    ...
    if (ch != 0) {
        *valid = TRUE;
    } else if (!iskeypad(scan) && altseq <= 0) {
        *valid = FALSE;
    }
#endif
    if (ch == '\r')
