...
5.0.0ベースのテストリリースです。現状windows/ttyのみの対応です。未翻訳の部分が残っています。コンパイル警告も残っています。

## JNetHack 3.6.* からの変更点

JNetHack 3.6.* から変更された主な点は以下の通りです。

 * ソースコードはUTF-8に変更しています。事前の変換は不要になりました。
   * 実行時は今まで通りSJISで実行されます。
