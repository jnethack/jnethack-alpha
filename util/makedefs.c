...
build_savebones_compat_string()
{
    ...
    Sprintf(eos(save_bones_compat_buf), "s %lu.%lu.%lu through %d.%d.%d",
            ...
            PATCHLEVEL);
#else
    Sprintf(eos(save_bones_compat_buf), "%lu.%lu.%luから%d.%d.%d",
            ((uver & 0xFF000000L) >> 24), ((uver & 0x00FF0000L) >> 16),
            ((uver & 0x0000FF00L) >> 8), VERSION_MAJOR, VERSION_MINOR,
            PATCHLEVEL);
#endif
...
#else
    Sprintf(eos(save_bones_compat_buf), "%d.%d.%dのみ", VERSION_MAJOR,
            VERSION_MINOR, PATCHLEVEL);
#endif
#endif
}
...
*/
    DEV_RANDOM "による強力な疑似乱数の種",
#else
/*JP
    "strong PRNG seed available from DEV_RANDOM",
*/
    "DEV_RANDOMによる強力な疑似乱数の種",
#endif
#else
#ifdef WIN32
/*JP
...
*/
    "CNG BCryptGenRandom()による強力な疑似乱数の種",
#endif
#endif  /* DEV_RANDOM */    
#else   /* ISAAC64 */
#ifdef RANDOM
...
*/
    "ハイライト付きのウィンドウによるステータス",
#else
/*JP
    "status via windowport without highlighting",
*/
    "ハイライトなしのウィンドウによるステータス",
#endif
#ifdef SUSPEND
    "suspend command",
#endif
...
*/
        Sprintf(buf, "デフォルトは\"%s\".", DEFAULT_WINDOW_SYS);
