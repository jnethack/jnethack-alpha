...
folding_japanese(
     ...
     int pos)
{
    char ss[BUFSZ * 4], s1[BUFSZ * 4], s2[BUFSZ * 4];
    static char newstr[BUFSZ * 4]; /* may be enough */

    if (strlen(str) > BUFSZ * 3) /*安全のため*/
        return str;
...
#else
    if (displen(bp) < CO) 
        strscpy_japanese(gt.toplines, bp, TBUFSZ);
    else
        strscpy_japanese(gt.toplines, folding_japanese(bp, CO - 2), TBUFSZ);
#endif
